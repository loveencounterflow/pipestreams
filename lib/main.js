...
(function() {
  'use strict';
  var $pass_through, $pull_drain, $pull_split, $pull_utf8_decoder, $stringify, $take, $values, CND, CP, Event_emitter, FS, STPS, alert, badge, debug, echo, help, info, log, map, new_pushable, pull, pull_async_map, pull_through, return_id, rpr, urge, warn, whisper,
    slice = [].slice;
  ...
  this._new_event_emitter = function() {
    ...
    R.emit = function() {
      ...
      return _emit.apply(null, [event_name].concat(slice.call(P)));
    };
    return R;
  };
  ...
  this._mixin_event_emitter = function(method) {
    ...
    method.on = function() {
      var P, event_name;
      event_name = arguments[0], P = 2 <= arguments.length ? slice.call(arguments, 1) : [];
      return emitter.on.apply(emitter, [event_name].concat(slice.call(P)));
    };
    method.emit = function() {
      var P, event_name;
      event_name = arguments[0], P = 2 <= arguments.length ? slice.call(arguments, 1) : [];
      return emitter.emit.apply(emitter, [event_name].concat(slice.call(P)));
    };
    return method;
  };
...
;
  ...
  this._new_file_sink_using_stps = function(path_or_stream) {
    ...
    R.on = function() {
      var P, name;
      name = arguments[0], P = 2 <= arguments.length ? slice.call(arguments, 1) : [];
      return emitter.on.apply(emitter, [name].concat(slice.call(P)));
    };
    stream.on('finish', function() {
      if (!emitter.listeners('stop', true)) {
  ...
  this.$gliding_window = function(width, method) {
    ...
    return this.$('null', (function(_this) {
      return function(new_data, send_) {
        send = send_;
        if (new_data != null) {
          push(new_data);
