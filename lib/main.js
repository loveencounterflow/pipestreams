...
(function() {
  'use strict';
  var $pass_through, $pull_drain, $pull_split, $pull_utf8_decoder, $stringify, $take, $values, CND, CP, Event_emitter, FS, STPS, alert, badge, debug, echo, help, info, log, map, new_pushable, pull, pull_async_map, pull_cont, pull_many, pull_through, return_id, rpr, urge, warn, whisper;
  ...
  this.new_event_collector = function(for_event_name, method) {
    ...
    switch (arity = arguments.length) {
      ...
      case 2:
        ...
        break;
      default:
        throw new Error(`expected 0 or 2 arguments, got ${arity}`);
    }
    ...
    R.add = function(emitter) {
      ...
      if (R._emitters.has(emitter)) {
        throw new Error("got duplicate emitter");
      }
...
;
      ...
      if (data != null) {
        if (is_first) {
          ...
          switch (type) {
            ...
            case 'buffer':
              if (joiner != null) {
                throw new Error(`joiner not supported for buffers, got ${rpr(joiner)}`);
              }
              break;
            default:
  ...
  this.$gliding_window = function(width, method) {
    ...
    if ((CND.type_of(width)) !== 'number') {
      throw new Error(`expected a number, got a ${type}`);
    }
    section = [];
    send = null;
    push = function(x) {
  ...
  this._spawn = function(command, settings) {
    var $ensure_event_order, arity, confluence, cp, on_error, signal_pipeline, source, spawn_pipeline, stderr, stderr_pipeline, stdout, stdout_pipeline;
    switch (arity = arguments.length) {
      case 1:
      case 2:
        null;
    ...
    if ((on_error = settings.on_error) != null) {
      delete settings.on_error;
    }
    ...
    (() => {
      ...
      cp.on('disconnect', function() {
        return signal_buffer.push(['disconnect', null]);
      });
      cp.on('error', function(error) {
        return signal_buffer.push(['error', error != null ? error : null]);
      });
      cp.on('exit', (code, signal) => {
        ...
        return signal_buffer.push(['exit', {code, signal}]);
      });
       ...
       */
      return signal_pipeline.push(pull_cont(function(handler) {
        return cp.on('close', function() {
          return handler(null, this.new_value_source(signal_buffer));
        });
      }));
    })();
    ...
    $ensure_event_order = () => {
      ...
      std_buffer = [];
      command_sent = false;
      return this.$('null', function(event, send) {
        ...
        if (event != null) {
          ...
          return cp_buffer.push(event);
        } else {

          /* Send all buffered CP events: */
          results = [];
          while (cp_buffer.length > 0) {
            results.push(send(cp_buffer.shift()));
          }
          return results;
        }
      });
    };
    confluence = pull_many([pull(this.new_value_source([['command', command]])), pull(...stdout_pipeline), pull(...stderr_pipeline), pull(...signal_pipeline)]);
