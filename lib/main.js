...
(function() {
  'use strict';
  var $pass_through, $pull_drain, $pull_split, $pull_utf8_decoder, $stringify, $take, $values, CND, CP, Event_emitter, FS, STPS, XXX_idx, alert, badge, debug, echo, help, info, log, map, new_pushable, pull, pull_async_map, pull_through, return_id, rpr, urge, warn, whisper,
    slice = [].slice;
...
;
  ...
  this.$drain = function(on_end) {
    ...
    R.on = function() {
      var P;
      P = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      return emitter.on.apply(emitter, P);
    };
    if (on_end != null) {
      R.on('stop', on_end);
    }
